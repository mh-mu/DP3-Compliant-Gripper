...
zarr_path = '/home/mh2595/workspace/implicit_force_simulation/third_party/3D-Diffusion-Policy/3D-Diffusion-Policy/data/real-world_touch_expert.zarr'
zarr_dataset = zarr.open(zarr_path, mode='r+')

# Access the 'data' group
data_group = zarr_dataset['data']
# Iterate through subfolders in 'data' and remove the last 450 arrays
for array_name in data_group.array_keys():
    array = data_group[array_name]
    if isinstance(array, zarr.core.Array):
        array.resize((array.shape[0] - 450,) + array.shape[1:])
