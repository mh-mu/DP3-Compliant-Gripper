...
from diffusion_policy_3d.model.vision.pointnet_extractor import DP3Encoder, DP3CompliantEncoder, DP3PcdCompliantEncoder, DP3RealworldEncoder
...
class DP3Compliant(BasePolicy):
    def __init__(self, 
        ...
        obs_encoder = DP3CompliantEncoder(observation_space=obs_dict,
                                                   img_crop_shape=crop_shape,
                                                out_channel=encoder_output_dim,
        ...
        cprint(f"[DiffusionUnetHybridPointcloudPolicyCompliant] use_compliant_image: {self.use_compliant_image}", "yellow")
...
class DP3PcdCompliant(BasePolicy):
    def __init__(self, 
            shape_meta: dict,
            noise_scheduler: DDPMScheduler,
            horizon, 
        ...
        obs_encoder = DP3PcdCompliantEncoder(observation_space=obs_dict,
                                                img_crop_shape=crop_shape,
                                                out_channel=encoder_output_dim,
                                                use_compliant_image=use_compliant_image,
...
class DP3Realworld(BasePolicy):
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        """
        # normalize input
        # print('predict action', obs_dict['wrist_img'].shape)
        nobs = self.normalizer.normalize(obs_dict)
        # use only the rgb image of the combined image (first 3 channels)
        # if not self.use_compliant_image:
        #     nobs['combined_img'] = nobs['combined_img'][..., :3,:,:]
