...
class RealWorldMilticamDataset(BaseDataset):
    def __init__(self,
            zarr_path, 
            horizon=1,
            pad_before=0,
        ...
        self.task_name = task_name
        # self.replay_buffer = ReplayBuffer.copy_from_path(
        #     zarr_path, keys=['state', 'action', 'force', 'wrist_img', 'gripper_img', 'third_view_img'])
        self.replay_buffer = ReplayBuffer.copy_from_path(
            zarr_path, keys=['state', 'action', 'force', 'third_view_img'])
        val_mask = get_val_mask(
            n_episodes=self.replay_buffer.n_episodes, 
            val_ratio=val_ratio,
            seed=seed)
    ...
    def _sample_to_data(self, sample):
        ...
        # wrist_img = sample['wrist_img'][:,].astype(np.float32).transpose(0, 3, 1, 2) # TODO: where should be transpose be
        # gripper_image = sample['gripper_img'][:,].astype(np.float32).transpose(0, 3, 1, 2)
        third_view_img = sample['third_view_img'][:,].astype(np.float32).transpose(0, 3, 1, 2)
        force = sample['force'][:,].astype(np.float32)

        data = {
            'obs': {
                # 'wrist_img': wrist_img, # T, 1024, 6
                # 'gripper_img': gripper_image,
                'third_view_img': third_view_img,
                'state': state, # T, D_pos
                'force': force,
            },
            'action': sample['action'].astype(np.float32) # T, D_action
